...
const TITLE_COPY = {
  en: "Ithaca Agency | Automation, Integrations & Custom Software in Greece",
  el: "Ithaca Agency | Automation, Integrations & Custom Software in Greece",
};
const DESCRIPTION_COPY = {
  en: "Ithaca Agency designs custom software, system integrations, AI automation, and workflow solutions for teams in Greece and remote.",
  el: "Ithaca Agency designs custom software, system integrations, AI automation, and workflow solutions for teams in Greece and remote.",
};
...
function getLangFromUrl() {
  try {
    ...
    if (!lang) return null;
    if (lang === "en" || lang === "el") return lang;
  } catch (_) {}
  return null;
}
...
function applyLang(lang) {
  ...
  document.title = isEl ? TITLE_COPY.el : TITLE_COPY.en;
  const description = isEl ? DESCRIPTION_COPY.el : DESCRIPTION_COPY.en;
  ...
  setMetaContent('meta[property="og:locale"]', isEl ? "el_GR" : "en_US");
  ...
  // Aria labels
  $$('[data-i18n-aria-en][data-i18n-aria-el]').forEach((node) => {
    const value = isEl
      ? node.getAttribute("data-i18n-aria-el")
      : node.getAttribute("data-i18n-aria-en");
    if (value == null) return;
...
function initContactForm() {
  ...
  const copy = {
    ...
    el: {
      required: "Παρακαλώ συμπλήρωσε όλα τα πεδία.",
      invalid: "Παρακαλώ βάλε ένα έγκυρο email.",
      sending: "Αποστολή...",
  ...
  function t(key) {
    const lang = document.documentElement.getAttribute("lang") === "el" ? "el" : "en";
...
function initShowreelDocking() {
  ...
  function setPlaceholderHeight(height) {
    placeholder.style.height = height > 0 ? `${Math.ceil(height)}px` : "0px";
  }
  ...
  function dockCard() {
    if (dismissed || isDocked()) return;
    setPlaceholderHeight(measureCardHeight());
  ...
  const io = new IntersectionObserver(
    (entries) => {
      ...
      if (isSectionIntersecting && dismissed) {
        clearDismissedState();
      }
      evaluateDockState();
    },
...
function initActiveSectionNav() {
  const sectionIds = ["top", "services", "work", "process", "about", "contact"];
  ...
  const links = $$(
    '.site-header .nav a[href^="#"], .mobile-nav__links a[href^="#"], .site-header .brand[href="#top"], .mobile-brand[href="#top"]'
  );
  ...
  const observer = new IntersectionObserver(
    (entries) => {
      ...
      setActive(bestRatio > 0 ? bestId : pickByPosition());
    },
    {
...
function initMobileNav() {
  ...
  function open() {
    ...
    // Safety fallback if animations fail silently
    setTimeout(() => {
      if (isOpen) showInstant();
    }, 480);
  ...
  toggle.addEventListener("click", () => {
    if (Date.now() - lastToggleTs < 500) return;
    toggleMenu();
  });
